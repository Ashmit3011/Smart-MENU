...
st.markdown("""
    <style>
        .nav { display: flex; gap: 1rem; background: #f0f2f6; padding: 0.75rem 1rem; border-radius: 0.5rem; margin-bottom: 1rem; }
    ...
    <div class='nav'>
        ...
        <a href='/track_order' target='_self'>📦 Track Orders</a>
        <a class='active' href='/admin' target='_self'>🧑‍🍳 Admin</a>
    </div>
    ...
    if completed:
        ...
        for order in completed:
            with st.expander(f"🧾 Order #{order['id']} - Table {order['table']}"):
                total = sum(item['qty'] * item['price'] for item in order["cart"])
                for item in order["cart"]:
                    st.markdown(f"- {item['name']} x {item['qty']} = ₹{item['qty'] * item['price']}")
